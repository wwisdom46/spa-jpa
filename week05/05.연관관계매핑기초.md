...
* 객체 연관관계 vs. 테이블 연관관계
    ...
    * 객체를 양방향으로 참조하려면, 단방향 연관관계를 2개 만들어야 한다.
        * 참조를 통한 연관관계는 언제나 단방향이다. 객체 간에 연관관계를 양방향으로 만들고 싶으면, 반대쪽에도 필드를 추가해서 참조를 보관래야 한다. → 서로 다른 단방향 관계 2개가 생긴다.
...
* 연관관계의 주인은 테이블에 외래 키가 있는 곳으로 정해야 한다. (자기 테이블에 있는 외래 키를 관리하면 되므로)
